...
// ****lab1 code begin****
    setThreadID();
    ++threadCount;
    ASSERT(threadCount <= MAXIMUM_THREAD_AMOUNT);
    setUserID();
// ****lab1 code end****
// ****lab2 code begin****
...
{
// ****lab1 code begin****
    DEBUG('t', "Deleting thread \"%s\", threadID=%d, userID=%d\n", name, threadID, userID);
    threadLink[threadID]=NULL;
    --threadCount;
// ****lab1 code end****
...
int Thread::setThreadID()
{
    threadID=-1;
    for(int i=0;i<MAXIMUM_THREAD_AMOUNT;++i)
    {
        if(threadLink[i]==NULL)
        {
            threadLink[i]=this;
            threadID=i;
            break;
...
// ****lab1 code end****
// ****lab2 code begin****
int Thread::setPriority(int threadPriority)
{
    if(threadPriority>2||threadPriority<0) //if priority error
        priority=2; //then give it a lowest priority
    else
...
{
// ****lab1 code begin****
    DEBUG('t', "Forking thread \"%s\"(threadID=%d, userID=%d) with func = 0x%x, arg = %d\n",
	  name, threadID, userID, (int) func, arg);
// ****lab1 code end****
